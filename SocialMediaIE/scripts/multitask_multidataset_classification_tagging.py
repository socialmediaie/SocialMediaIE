...
from SocialMediaIE.data.multi_task_reader import create_classification_tagging_vocab, read_datasets
from SocialMediaIE.evaluation.evaluate_model import evaluate_multiple_data
from SocialMediaIE.models.multi_task_classification_tagging_models import (
    MultiTaskCRFTaggerAndClassifier,
...
def run(args):
    ...
    for task in TASKS:
        if task.task_type in TAGGING_TASKS:
            ...
            raise NotImplementedError(f"task_namespace={task.task_type} not yet supported.")
    ...
    if not TEST_MODE:
        ...
        vocab = create_classification_tagging_vocab([train_dataset, validation_dataset])
    ...
    model = MultiTaskCRFTaggerAndClassifier(word_embeddings, encoders, vocab, TASKS)
